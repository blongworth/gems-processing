...
tar_load(c(rga_binned, rga_interp_test, rga_oxygen))
```
...
```{r}
e <- rga_interp_test |>
  select(time_grid, mass_32_40_low, mass_32_40_high) |>
  pivot_longer(
...
ggplot(aes(time_grid, mass_32_40, color = inlet)) +
  geom_line()
...
```{r}
e <- ggplot(rga_oxygen, aes(timestamp, mass_32_40_high_diff)) +
  geom_line()

f <- ggplot(rga_oxygen, aes(timestamp, par)) +
  geom_line()
