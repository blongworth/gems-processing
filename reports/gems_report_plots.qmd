...
#| fig-height: 5.5
#| fig-cap: RGA signals for masses of interest.  Each mass is measured approximately every 10 s.  A five day period of the deployment was chosen to highlight diel cycles. Each signal is normalized to mass 40 (Argon) to control for effects of temperature, pressure, and biofouling.  The mass 34 signal is likely an interference with Oxygen, rather than a true hydrogen sufide signal.
...
#| fig-cap: Comparison of QMS signal noise for argon-normalized oxygen between 2022 (A) and 2025 (B) deployments.Signal noise was greatly reduced for the 2025 deployment, improving the accuracy and limit of detection for gradient determination.
...
#|   This figure shows un-normalized (top) and normalized (bottom) Nitrogen (mass 28)
...
#| fig-cap: A record of the argon-normalized oxygen signal for the duration of the deployment.
rga_binned_wide |>
  arrange(timestamp) |>
...
#|   showing a clear gradient between the high and low inlets.
...
```{r}
#| label: fig-cal
#| fig-cap: Oxygen calibration data for the SeapHOx codeployment. SeapHOx optode data is compared to GEMS mass 32:40. The line shows linear model fit.
#| warning: false
...
#| fig-cap: SeapHOx optode and fitted GEMS oxygen over the codeployment time period. Fitted RGA Oxygen data shows a reasonable match to the SeapHOx optode data for the codeployment period.
#| warning: false
#|
cal_plot_df |>
  pivot_longer(
...
#| fig-cap: >
#|   Hourly oxygen flux for the duration of the deployment.
#|   estimated daily light integral (DLI) and measured seawater temperature for comparison.
...
(gop / dpp / tp) + plot_layout(heights = c(3, 2, 2))
```
...
#| label: fig-flux-zoom
#| fig-cap: >
#|   Oxygen flux with predicted PAR for comparison
...
(jfp / jpp) + plot_layout(heights = c(3, 1))
```
...
#| fig-cap: >
#|   GEMS oxygen concentration and gradient aggregated across the deployment by hour of day. Hour is adjusted to the mean solar noon for the deployment. Mean oxygen concentration across both inlets.
...
#| label: fig-flux-diel
#| fig-cap: >
#|   GEMS oxygen flux and predicted par aggregated hourly for the duration of the deployment. Hour is adjusted to the mean solar noon for the deployment.
#| fig-height: 5
(flp / parp) + plot_layout(heights = c(3, 1))
```
...
#| fig-cap: >
#|   GEMS oxygen flux and predicted par aggregated hourly and separated by month. Hour is adjusted to the mean solar noon for the deployment. June data is not shown due to bias from only 3 days of data acquisition.
#| fig-height: 5
...
```{r}
#| label: fig-flux-nem-hourly
#| fig-cap: >
#|   Oxygen flux integrated by month for the deployment period
#|   to provide an estimate of net ecosystem metabolism (NEM).
#|   The benthic system at Naushon Island is close
...
ggplot(monthly_nem, aes(month_factor, nem_mmol_m2_day)) +
  geom_col() +
  geom_hline(yintercept = 0) +
  labs(
    x = NULL,
...
```{r}
#| label: fig-flux-nem-monthly
#| fig-cap: Daily-integrated NEM for the deployment period. Daily NEM is variable, but is close to net-neutral over time.
#| fig-height: 2.75
...
#| label: fig-eelgrass
#| fig-cap: >
#|   Naushon eelgrass shoot biomass (top) and shoot length (bottom) data by year.
