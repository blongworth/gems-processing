...
#| fig-cap: GEMS DIC flux relationship to oxygen flux. DIC flux is determined for the period of the SeapHOx codeployment, using measured seapHOx pH and calibrated GEMS CO₂, assuming uniform pH between top and bottom inlets, and using CO2SYS for carbonate system calculations. The corrected DIC flux holds high-inlet alkalinity constant and predicts low-inlet DIC from low-inlet CO₂.
...
hmp <- monthly_stats |>
  ggplot(aes(solar_hour, ox_mean_umol_l_mean, color = month)) +
  geom_line() +
  geom_pointrange(
    aes(
      ymin = ox_mean_umol_l_mean - ox_mean_umol_l_se,
...
hgp <- monthly_stats |>
  ggplot(aes(solar_hour, ox_gradient_umol_l_m_mean, color = month)) +
  geom_line() +
  geom_pointrange(
    aes(
      ymin = ox_gradient_umol_l_m_mean - ox_gradient_umol_l_m_se,
